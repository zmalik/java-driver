...
public class TopologyDiagnosticGenerator {
  ...
  public TopologyDiagnosticGenerator(@NonNull Supplier<Metadata> metadataSupplier) {
    Objects.requireNonNull(metadataSupplier, "session must not be null");
   ...
   *
   * <p>For a more fine-grained health report, use {@link RingDiagnosticGenerator}, which is capable
   * of determining if the cluster is still healthy enough, even when some nodes are down.
   */
  public TopologyDiagnostic generate() {
    Map<UUID, Node> nodes = metadataSupplier.get().getNodes();

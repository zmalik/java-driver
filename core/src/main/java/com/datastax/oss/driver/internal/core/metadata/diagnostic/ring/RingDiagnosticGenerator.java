...
public interface RingDiagnosticGenerator {
   ...
   *
   * <p>The token ring should be considered {@linkplain RingDiagnostic#isAvailable() healthy} if all
   * {@linkplain TokenRange token ranges} are available; a token range is considered available if
   * the configured {@linkplain ConsistencyLevel consistency level} is achievable on it for the
   * configured {@linkplain KeyspaceMetadata keyspace}.
   *
